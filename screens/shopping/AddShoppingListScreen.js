...
export default function AddShoppingListScreen({ navigation }) {
  ...
  const handleAddList = async () => {
    if (!listName.trim()) {
      Alert.alert('Error', 'Please enter a shopping list name');
      return;
    }
      ...
      if (error) {
        ...
        Alert.alert('Success', 'Shopping list created successfully', [
          { text: 'OK', onPress: () => {
            ...
            navigation.navigate('ShoppingList', { refresh: Date.now() });
          }}
        ]);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.content}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Add Shopping List</Text>
