...
import { Check, ChevronLeft, Search, Share as ShareIcon, Plus, ArrowUpDown, CheckCircle2, Circle, Trash2 } from 'lucide-react-native';
...
export default function ShoppingListItemsScreen({ route, navigation }) {
  ...
  const handleAddItem = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        await fetchShoppingItems(); // Refresh the list
      }
    } catch (error) {
      console.error('Error adding item:', error);
      Alert.alert('Error', 'Failed to add item');
  ...
  useEffect(() => {
    if (shouldResort || sortedItems.length === 0) {
  ...
  const filteredItems = useMemo(() => {
    let items = sortedItems.length > 0 ? sortedItems : shoppingItems;
