import { StyleSheet, Text, View, TouchableOpacity, ScrollView, TextInput, ActivityIndicator, Modal, Alert } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Check, Search, ArrowUpDown, CheckCircle2, ArrowRight } from 'lucide-react-native';
...
export default function OBSelectCountry({ navigation }) {
  ...
  const handleCountrySelect = async (selectedCountry) => {
    ...
    if (user?.id) {
      try {
        const { error } = await userService.updateUserCountry(user.id, selectedCountry.name, selectedCountry.code);

        if (error) {
          console.error('Failed to save country to database:', error);
          // Still allow the user to continue, but log the error
        } else {
          console.log('Country saved to database successfully');
        }
      } catch (error) {
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
        ...
        {isCountrySelected && (
          <View style={[styles.currentSelectionContainer, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.currentSelectionLabel, { color: theme.colors.textSecondary }]}>Selected:</Text>
            <Text style={[styles.currentSelectionValue, { color: theme.colors.text }]}>{country.name}</Text>
          </View>
        )}

        {!isCountrySelected && (
          <Text style={[styles.subtitle, { color: theme.colors.text }]}>Please select your country to continue</Text>
        )}
      ...
      <View style={[styles.bottomCTA, { backgroundColor: theme.colors.background }]}>
        <TouchableOpacity
          style={[
            styles.nextButton,
            {
              backgroundColor: isCountrySelected ? theme.colors.primary : theme.colors.border,
              opacity: isCountrySelected ? 1 : 0.6
            }
          ]}
          onPress={handleNext}
        ...
        >
          <Text style={[
            ...
            { color: isCountrySelected ? '#fff' : theme.colors.textSecondary }
          ]}>
            Next
          </Text>
          <ArrowRight size={20} color={isCountrySelected ? '#fff' : theme.colors.textSecondary} />
        </TouchableOpacity>
      </View>
